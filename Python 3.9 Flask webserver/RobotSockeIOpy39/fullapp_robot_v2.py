...
# MQTT for Stream Video over local network
MQTT_BROKER= "localhost"
# Topic on which frame will be published
MQTT_SEND = "robotcar/camera"
# Object to capture the frames
# Phao-MQTT Clinet
client_video = mqtt_client.Client(mqtt_client.CallbackAPIVersion.VERSION2)

client_video.connect(MQTT_BROKER)
    ...
    try:
        while(cap.isOpened() and event.is_set()):
            ...
            if ret:
                ...
                # ----------------------MQTT--------------
                frame1 = imutils.resize(img, width=480)
                # Encoding the Frame
                ...
                client_video.publish(MQTT_SEND, jpg_as_text)
                # end = time.time()
                # t = end - start
                # fps = 1 / t
                #-------------------End MQTT------------------------
