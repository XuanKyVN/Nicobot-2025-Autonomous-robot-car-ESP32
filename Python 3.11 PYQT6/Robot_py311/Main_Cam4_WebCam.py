...
class VideoThread(QThread):
    ...
    def __init__(self, index):
        # super().__init__()
        super(VideoThread, self).__init__(parent=None)
    ...
    def run(self):
        model1 = YOLO("yolo11n.pt") # Use for Intel Realsens Cam
        #self.model = YOLO(r"C:\Users\Admin\PythonLession\RobotSocketIOMqtt\Robot_prj1\model\yolo11n.pt")   # Use for Normal Camera

        self.model= YOLO(r"model\best_robot_v3.pt")
        ...
        if self.index == 1:  # AFT CAMERA
            ...
            while self._run_flag_aft:
                ...
                results = self.model.predict(frame, device=[0], conf =0.5)
            ...
            if (self._run_flag_fwd):
                ...
                try:
                    while self._run_flag_fwd:
                        ...
                        # ---------------YOLO-------------------
                        results = model1.predict(color_image, device=[0])  # detect all object
    ...
    def video_receivedata(self, data):
        ...
        if ("set_video_path" in data.keys()):
            ...
            self.model = data["set_pretrain"]
            print( self.model)
            if len(data["set_video_path"]) > 1:  # Normal Video
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.uic.bt_select_ptrain.clicked.connect(self.selectPretrain)
        self.uic.txt_ptrain.setText(r"C:\Users\Admin\PythonLession\RobotSocketIOMqtt\Robot_prj1\model\best_robot_v3.pt")
...
#-------------------------------------Safety--------------------------------------------
        self.uic.frame_set_safety.hide()
        self.uic.bt_show_safty_frame.clicked.connect(self.uic.frame_set_safety.show)
        self.uic.bt_hide_safety_frame.clicked.connect(self.uic.frame_set_safety.hide)

        self.uic.bt_set_img_safty.clicked.connect(self.set_safety_img_process)  # if available people within set distance Robot car will stop

        self.uic.bt_set_Rplidar_safety.clicked.connect(self.set_safety_Rplidar_process)  # if any obstruction in range 0o (FWD) or 180o   (AFT) Robot will stop

        self.bt_enb_img_dist = False
        self.bt_enb_Rplidar_dist = False
        self.robot_state = False
        self.moving_status = "STOP"


#-----------------------Image Process Control-------------------------------
        self.enb_ctrl_Image = False
        self.uic.frame_set_img_control.hide()
        self.uic.bt_show_img_control.clicked.connect(self.uic.frame_set_img_control.show)
        self.uic.bt_hide_img_control.clicked.connect(self.uic.frame_set_img_control.hide)

        self.uic.bt_set_img_ctrl.clicked.connect(self.set_img_control)
    ...
    def read_mqttsetup(self):
        ...
        if exit:
            with open('mqttsetup.json', 'r') as mqttjsonfile:
                ...
                self.uic.text_port.setText(str(json_object['port']))
    ...
    def ex_operate_robot(self, n):
        ...
        if (n == 1):  # Stop
            ...
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Stop Robot car")

        if (n == 2):  # Start
            self.datacontroljson = {"robot_state": True}
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Start Robot Car")

        if (n == 3):  # fwd
            self.datacontroljson = {"motorspeed_cmd": speed, "direct_cmd": 1}
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Move FWD")
        if (n == 4):  # bwd
            self.datacontroljson = {"motorspeed_cmd": speed, "direct_cmd": 2}
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Move BWD")

        if (n == 5):  # left
            self.datacontroljson = {"motorspeed_cmd": speed, "direct_cmd": 3}
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Move LEFT")

        if (n == 6):  # right
            self.datacontroljson = {"motorspeed_cmd": speed, "direct_cmd": 4}
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Move RIGHT")

        if (n == 7):  # fwdright
            self.datacontroljson = {"motorspeed_cmd": speed, "direct_cmd": 5}
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Move FWD Right")

        if (n == 8):  # fwdleft
            self.datacontroljson = {"motorspeed_cmd": speed, "direct_cmd": 6}
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Move FWD Left")

        if (n == 9):  # bwdleft
            self.datacontroljson = {"motorspeed_cmd": speed, "direct_cmd": 7}
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Move BWD Left")

        if (n == 10):  # bwd right
            self.datacontroljson = {"motorspeed_cmd": speed, "direct_cmd": 8}
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Move BWD Right")

        if (n == 11):  # spinright
            self.datacontroljson = {"motorspeed_cmd": speed, "direct_cmd": 9}
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Move SPIN RIGHT")

        if (n == 12):  # spin left
            self.datacontroljson = {"motorspeed_cmd": speed, "direct_cmd": 10}
            self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Move SPIN LEFT")

    def speeddisplay(self, value):
        self.uic.lbl_speeddisplay.setText(str(value))
        self.speed_change(value)


    def speed_change(self, value):
        jsondata = {"speed": int(value)}
        if (self.robot_state == True):
            if self.lora_mqtt_mode == False:  # mqtt select
                if self.flag_mqtt == True:

                    self.signal_to_mqttthread.emit(jsondata)  # HMI Send Json Data To MQTT Thread
                else:  # Lora select
                    self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "MQTT is not stated")
                    print("MQTT is not started")
            else:
                if self.flag_lora == True:  # Lora select  , Serial
                    ...
                    self.signal_to_lorathread.emit(jsondata)  # HMI Send Json Data To Lora Thread
                else:
                    self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "LORA is not stated")
                    print("LORA is not started")
    ...
    def sendatato_hmi(self, datasim1):
        ...
        if ('robot_state' in datasim1.keys()):
            ...
            if (datasim1['robot_state'] == True):
                ...
                self.uic.lbl_robotready.setPixmap(QPixmap("png/notready.png"))
                self.robot_state = False
            if (datasim1['robot_mode'] == True):
                self.uic.lbl_auto_man.setPixmap(QPixmap("png/auto.png"))
            else:
                self.uic.lbl_auto_man.setPixmap(QPixmap("png/manu.png"))
            ...
            if (datasim1['mqttstatus']):
                ...
                self.uic.lbl_mqtt_status.setPixmap(QPixmap("png/disconnect.png"))

            if (datasim1['L_Hcurrent']):
                self.uic.lbl_Lhighcurrent.setText("ABNORMAL")
            else:
                self.uic.lbl_Lhighcurrent.setText("NORMAL")
            if (datasim1['R_Hcurrent']):
                self.uic.lbl_Rhighcurrent.setText("ABNORMAL")
            else:
                self.uic.lbl_Rhighcurrent.setText("NORMAL")

            if (datasim1['low_volt_ctr']):
                self.uic.lbl_lowvolt_ctr.setText("ABNORMAL")
            else:
                self.uic.lbl_lowvolt_ctr.setText("NORMAL")
            if (datasim1['low_volt_motor']):
                self.uic.lbl_lowvolt_motor.setText("ABNORMAL")
            else:
                self.uic.lbl_lowvolt_motor.setText("NORMAL")




        elif ("PS2_status" in datasim1.keys()):
            if (datasim1['PS2_status']):
                self.uic.lbl_ps2_remote.setText("ABNORMAL")
            else:
                self.uic.lbl_ps2_remote.setText("NORMAL")

            self.uic.lcd_humidity.display(datasim1["humi"])
            self.uic.lcd_temperature.display(datasim1["temp"])
            self.uic.lcd_fwd_dist.display(datasim1["fwd_dist"])
            self.uic.lcd_aft_dist.display(datasim1["aft_dist"])
    ...
    def receive_data_from_videothread_aft(self, data):
        ...
        if (self.enb_ctrl_Image and len(data["objectclasses"])>0):
            objclass = data["objectclasses"][0].strip()  # remove white space
            print(objclass)
            if (objclass =="forward"):
                if (self.robot_state == True and self.moving_status != "FWD"):
                    self.operate_robot(3)

            if (objclass =="backward"):
                if (self.robot_state == True and self.moving_status != "BWD"):
                    print("BWD Robot here2")
                    self.operate_robot(4)

            if (objclass =="left"):
                if (self.robot_state == True and self.moving_status != "LEFT"):
                    self.operate_robot(5)

            if (objclass =="right"):
                if (self.robot_state == True and self.moving_status != "RIGHT"):
                    self.operate_robot(6)

            if (objclass =="stop"):
                if (self.robot_state == True and self.moving_status != "STOP"):
                    self.operate_robot(1)
            if (objclass =="start"):
                if (self.robot_state == False or self.moving_status == "STOP"):
                    self.operate_robot(2)

            if (objclass == "speed20"):
                pass

            if (objclass == "speed30"):
                self.speed_change(30)
            if (objclass == "speed40"):
                self.speed_change(40)
            if (objclass == "speed50"):
            ...
            if (objclass == "speed80"):
                self.speed_change(80)
            if (objclass == "speed100"):
                self.speed_change(100)
    ...
    def receive_data_from_videothread_fwd(self, data):
        ...
        element = "person"
        self.Mindistance_cam_read,self.indexmatrix_cam_read = find_element_in_matrix(objclass, element, distance)

        if self.robot_state ==True:
            self.run_loop_safety_stop_robot()
    ...
    def select_control_mode_mqtt_lora(self):
        if self.uic.comboBox_Com_mode.currentIndex() == 0:
            ...
            self.lora_mqtt_mode = True  # Use LORA
    ...
    def list_usb_webcam(self):
        ...
        for device in devices:
            ...
            self.uic.plaintex_event.appendPlainText(
                str(datetime.now()) + ", " + str(device.index) + ", " + str(device.name))


    def set_aft_camera(self):
        cam = self.uic.txt_video_cam.text()
        pt = self.uic.txt_ptrain.text()
        if (cam != "") and (pt!=""):
            self.signal_to_videothread.emit({"set_video_path": cam, "set_pretrain":pt})
        else:
            print("Video path or pretrain path is not setup")
    ...
    def selectPretrain(self):
        ...
        dialog.setDirectory(r'C:\image')
        dialog.setFileMode(QFileDialog.FileMode.ExistingFiles)
        dialog.setNameFilter("Pretrain File (*.pt )")
        dialog.setViewMode(QFileDialog.ViewMode.List)
        if dialog.exec():
            filenames = dialog.selectedFiles()
            # print(filenames)
            if filenames:
                # self.file_list.addItems([str(Path(filename)) for filename in filenames])
                self.uic.txt_ptrain.setText(str(Path(filenames[0])))
...
#-----------SAFETY --------------------------------
    def set_safety_img_process(self):
        self.dist_img_set = int(self.uic.txt_dist_meter_img.text())
        if (self.uic.bt_enb_img_dist_radio.isChecked()):
            self.bt_enb_img_dist = True
            self.uic.plaintex_event.appendPlainText(
                str(datetime.now()) + ": " + "Enable Stop Car by Image processing")
        else:
            self.bt_enb_img_dist = False
            self.uic.plaintex_event.appendPlainText(
                str(datetime.now()) + ": " + "Disable Stop Car by Image processing")

        if (self.bt_enb_img_dist):
            '''
            if any people withing the distance  set Stop car
            
            '''
            pass
    def set_safety_Rplidar_process(self):   # IN Python Version 11, We can not use RP lidar

        self.dist_Rplidar_set = int(self.uic.txt_dist_centimet_rplidar.text())
        if (self.uic.bt_Rplidar_enb_radio.isChecked()):
            self.bt_enb_Rplidar_dist = True
            self.uic.plaintex_event.appendPlainText(
                str(datetime.now()) + ": " + "RPlidar is not support Python3.11 please use Python 3.9")
        else:
            self.bt_enb_Rplidar_dist = False
            self.uic.plaintex_event.appendPlainText(
            str(datetime.now()) + ": " + "RPlidar is not support Python3.11 please use Python 3.9")
    ...
    #------------------------------------------------
    def run_loop_safety_stop_robot(self):
        if (self.bt_enb_img_dist):
            '''
            if any people withing the distance  set Stop car

            '''
            if len(self.indexmatrix_cam_read)>0 :
                if (self.Mindistance_cam_read < self.dist_img_set):
                    self.uic.plaintex_event.appendPlainText(str(datetime.now()) + ": " + "Stop Robot due to find person in safety range")

                    self.operate_robot(1)
                    print("Stop Robot Due to find a person in the safety distance")

    def set_img_control(self):
        if(self.uic.bt_enb_img_ctrl_radio.isChecked()):
            self.enb_ctrl_Image =True
            self.uic.plaintex_event.appendPlainText(
                str(datetime.now()) + ": " + "Enable Image process Control")
        else:
            self.enb_ctrl_Image = False
            self.uic.plaintex_event.appendPlainText(
                str(datetime.now()) + ": " + "Disable Image process Control")
