...
class VideoThread(QThread):
    ...
    def run(self):
        ...
        # Open the video file
        #video_path = r"C:\Users\Admin\Documents\MATLAB\Assignment\Fish_Green_LED.mp4"
        video_path = 2
        ...
        while cap.isOpened():
            ...
            if success:
                frame = imutils.resize(frame, 760)
                results = model.predict(frame, device=[0])
                # Visualize the results on the frame
                annotated_frame = results[0].plot()
                self.change_pixmap_signal.emit(annotated_frame) # Send data image to HMI
...
#-----------------End of Main Program------------------------------------
    ...
    def config_videothread(self):
        ...
        self.VThread.change_pixmap_signal.connect(self.receive_image_fromvideo)
        self.signal_to_videothread.connect(self.VThread.video_receivedata)
        self.VThread.start()



    @pyqtSlot(np.ndarray)
    def receive_image_fromvideo(self, cv_img):
        """Updates the image_label with a new opencv image"""
        qt_img = self.convert_cv_qt(cv_img)
        self.uic.lbl_camera.setPixmap(qt_img)
