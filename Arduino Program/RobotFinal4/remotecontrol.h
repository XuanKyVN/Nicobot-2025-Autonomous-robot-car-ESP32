...
void receive_remote_ps2(){
  if(type == 1){ //DualShock Controller
    ...
    //--------------------------------------left and right----------------------
     ...
     if(ps2x.ButtonPressed(PSB_L2)){  // SPIN LEFT
          
          //g_CarState= enSpinLEFT;
          //Serial.println("Spin left, right motor run reverse, left motor run forward: " + String(CarSpeedControl));

          //   USE to Disable Ultrasonic Sensor Protection
          enb_stop_fwdsensor = false;
          enb_stop_aftsensor = false;
    ...
    //---------   Right------------
     ...
     if(ps2x.ButtonPressed(PSB_R2)){ // Spin right  
          
          //g_CarState= enSpinRIGHT;
          //Serial.println("Spin right, left motor run fwd, left motor run backward: " + String(CarSpeedControl));

          //   USE to Enable Ultrasonic Sensor Protection  
          enb_stop_fwdsensor = true;
          enb_stop_aftsensor = true;
        }
    ...
    if(ps2x.ButtonPressed(PSB_TRIANGLE))  {             
        ...
        // Send signal to MQTT to test milsec reaction time.
        client.publish("robotcar/test", "Test");
